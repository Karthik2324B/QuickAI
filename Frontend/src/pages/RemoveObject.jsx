  ...
  return (
    <div className='h-full flex flex-wrap items-start p-6 gap-4 text-slate-700 overflow-y-scroll '>
      ...
      <form onSubmit={onSubmitHandler} className='w-full border border-gray-200 bg-white rounded-lg max-w-sm p-4'>
        ...
        <textarea onChange={(e) => setObject(e.target.value)} value={object} rows={4} className='w-full p-2 px-3 mt-2 outline-none text-sm border border-gray-300 rounded-md' placeholder='e.g watch or spoon Only single object name' required />

        <button disabled={loading} className='w-full flex items-center justify-center gap-2 cursor-pointer rounded-lg text-sm bg-gradient-to-r from-[#417DF6] to-[#8E37EB] text-white px-4 py-2 mt-6 '>
          {
            loading ?
